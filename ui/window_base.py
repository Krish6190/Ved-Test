...
class VedWindowBase:
    def __init__(self, root: tk.Tk): # type: ignore
        ...
        # Absolute Static Target Coordinates
        ...
        # Calculate coordinates to anchor window cleanly above the bottom-right taskbar
        sw = self.root.winfo_screenwidth()
        sh = self.root.winfo_screenheight()
        self.x_pos = sw - self.default_width - 2
        self.y_pos_default = sh - self.default_height - 49
        self.y_pos_max = sh - self.max_height - 49

        # Initialize to resting default compact scale
        self.root.geometry(f"{self.default_width}x{self.default_height}+{self.x_pos}+{self.y_pos_default}")
    ...
    def _apply_window_style(self):
        """Ensures the window appears cleanly within the Windows Alt-Tab switching index and forces system focus."""
        try:
            self.root.update_idletasks()
            ...
            print(f"[ved] Shell style recovery failure: {e}")

    def _hide_from_screen_capture(self):
        """Prevents screen capture tools from registering the window bounding box."""
        try:
            self.root.update()
    ...
    def _show_tray_icon(self):
        ...
        canvas = tk.Canvas(self._tray, width=icon_size, height=icon_size, bg="#12131b", highlightthickness=1, highlightbackground="#313244")
        canvas.pack()
    ...
    def _start_drag(self, event):
        if getattr(self, "_drag_block_widgets", None) and event.widget in self._drag_block_widgets: return
